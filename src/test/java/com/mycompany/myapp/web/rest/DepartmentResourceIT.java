...
class DepartmentResourceIT {
    ...
    @Test
    @Transactional
    void getAllDepartmentsByJobHistoryIsEqualToSomething() throws Exception {
        ...
        em.flush();
        department.setJobHistory(jobHistory);
        jobHistory.setDepartment(department);
        departmentRepository.saveAndFlush(department);
        Long jobHistoryId = jobHistory.getId();
        // Get all the departmentList where jobHistory equals to jobHistoryId
        defaultDepartmentShouldBeFound("jobHistoryId.equals=" + jobHistoryId);

        // Get all the departmentList where jobHistory equals to (jobHistoryId + 1)
        defaultDepartmentShouldNotBeFound("jobHistoryId.equals=" + (jobHistoryId + 1));
    }
